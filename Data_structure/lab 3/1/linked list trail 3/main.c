...
/*
void remove_duplicates(SinglyLinkedListNode* head, int numbers_count) {
        ...
        while(current_state != NULL){
            ...
            while(next_state != NULL){
                if(current_state->data == next_state->data)
                    {
                    temp_state->next = next_state->next;
                    }
                else
...
int main()
{
    ...
    for (i = 0; i < numbers_count; i++) {
        int numbers_item = charValue[i];

        insert_node_into_singly_linked_list(&numbers, numbers_item);
    }

	//remove_duplicates(numbers->head,numbers_count);
